{:php-setup:}
$page_title = Globals::$site_name . ' - AdminTools';
$page_header = 'AdminTools';
$required_authority = 'X';
{:end-php-setup:}
<?php
/*
#doc-start
h1. AdminTools.tpl - administration tools to run in Off-Line Mode
...
<div class="box width-80">
  ...
  <?php if (Globals::$dbaccess->database_valid == 'T' && Globals::$dbaccess->model_mismatch == 'F'
      ...
      <p>System Ready to Go Online:
        <a href="AdminTools.tpl?admin_cmd=go_online" style="background:#c0ffa0;text-decoration:underline;">
  ...
  <?php elseif (Globals::$dbaccess->on_line != 'F'): ?>
      <p>System is <span class="bold">NOT</span> Offline:
        <a href="AdminTools.tpl?admin_cmd=go_offline" style="background:#c0ffa0;text-decoration:underline;">
          <span class="advisory">Click to Go Offline</span>
        </a>
      </p>
  <?php endif; ?>
  <ol>
    <li><a <?php echo set_color(); ?> href="AdminTools.tpl?admin_cmd=reload_db">Create Archive and/or Reload Database</a></li>
    <li><a <?php echo set_color(); ?> href="AdminTools.tpl?admin_cmd=rebuild_aclass_hashes_1">Rebuild AClass Hashes Table</a></li>
    <li><a <?php echo set_color(); ?> href="AdminTools.tpl?admin_cmd=display_site_state">Display Site Specific PHP Variables</a></li>
    <li><a <?php echo set_color(); ?> href="AdminTools.tpl?admin_cmd=phpinfo">Display PHP Info</a></li>
  </ol>
</div>
<?php
  ...
  foreach (StateMgt::$legal_state_names as $state_var) {
    ...
    foreach (StateMgt::$legal_state_values[$state_var] as $val) {
      if (Globals::$dbaccess->$state_var == $val) {
        ...
        $str .= "<td style=\"background:#0080c0\"><form action=\"AdminTools.tpl\" method=\"get\" accept-charset=\"utf-8\">"
...
?>
  <form action="AdminTools.tpl" method="get" accept-charset="utf-8">
...
switch (Globals::$rc->safe_get_admin_cmd) {
  ...
  case 'go_offline':
    ...
    break;
  case 'reload_db':
    IncludeUtilities::redirect_to('ReloadDB.tpl', basename(__FILE__) . ':' . __LINE__);
    break;
