...
/*
#doc-start
h1.  ImageObject - a Utility object for manipulating and displaying Images using GD2

Class ImageObject manages in-memory image data. Images in ImageObjects are automatically
...
* new ImageObject(path OR width, height) - creates a new ImageObject object from
the file located by the _path_ OR creates a new empty Image of size (width, height)
...
* save(path, format = png, $overwrite = FALSE) - Refuses
to clobber the original path unless _$overwrite_ is TRUE. Throws exceptions on problems
Legal formats are: png, jpg (or jpeg), gif, gd, and gd2
...
* shrink(max-dimension) - returns an ImageObject with width and height
...
image. Returns a new ImageObject
...
parameter.Returns a new ImageObject.
...
* protected getImage() - loads the image into memory from the given path

#doc-end
*/

class ImageObjectException extends Exception {}
...
class ImageObject {
  ...
  public function __construct()
  {
    ...
    if (count($args) == 2 && is_int($args[0]) && is_int($args[1])) {
      ...
      throw new ImageObjectException("ImageObject::__construct("
  ...
  {
    throw new ImageObjectException("ImageObject::__unset($name) - illegal action");
  }
  ...
  protected function allocatecolor($name, $red, $green, $blue, $alpha = 0)
  {
    ...
    if (!array_key_exists($name, $this->color_allocations)) {
      $this->color_allocations[ImageObject::rgbtocolor($red, $green, $blue)] =
...
//    }
  ...
  public function save($save_path, $format = 'png', $overwrite = FALSE)
  {
    if (!$this->image) {
      return FALSE;
    }
    if (!$overwrite && $save_path == $this->path) {
      throw new ImageObjectException("ImageObject::save($save_path) - save path collision");
    }
    $dir_path = dirname($save_path);
    if (!is_dir($dir_path)) {
      throw new ImageObjectException("ImageObject::save($save_path,...): directory '$dir_path' does not exist");
    }
    if (!is_writable($dir_path)) {
      throw new ImageObjectException("ImageObject::save($save_path,...): directory '$dir_path' not writeable");
    }
    switch ($format) {
      case 'jpeg': case 'jpg':
      case IMAGETYPE_JPEG:
      ...
      break;
      default:
      throw new ImageObjectException("ImageObject::save($save_path, $format, ...) - unsupported format");
    }
    return TRUE;
  }
...
//    $this->transparent(-1);
    $black = 0;
    $white = ImageObject::rgbtocolor(255,255,255);
    for ($x=0;$x<$width;$x++) {
      for ($y=0;$y<$height;$y++) {
  ...
  public function shrink($max_dimension)
  {
    ...
    $new_image = new ImageObject($new_width, $new_height);
...
//      imagecopyresampled($new_image->image, $this->image, 0, 0, 0, 0,
  ...
  {
    if ($this->emptyP()) {
      throw new ImageObjectException("ImageObject::display($max_dimension, $output_format): attempt to display void image");
    }
    extract($this->image_info);
    ...
    $center_x = .5, $center_y = .5)
  {
    ...
    if (is_string($overlay) && file_exists($overlay)) {
      $overlay = new ImageObject($overlay);
    }
    if ($overlay instanceof ImageObject) {
      if ($ratio > 0) {
  ...
  //  text "Copyrighted Image"
  private function checkWatermarkImage($watermark_image)
  {
    $opacity = 30;
    if ($watermark_image instanceof ImageObject && !$watermark_image->emptyP()) {
    ...
    // using freetype code
    // $watermark_image = new ImageObject($this->width, $this->height);
    $watermark_image = new ImageObject(strlen($font_text) * $char_box_width + $char_spacing,
        $char_box_height);
...
//    $white = $watermark_image->allocatecolor('white', 255,255,255,0);
  ...
  {
    list($watermark_image, $opacity) = $this->checkWatermarkImage($watermark_image);
