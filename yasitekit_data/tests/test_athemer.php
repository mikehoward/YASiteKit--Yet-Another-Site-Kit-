...
if ($test_yat_scanner) {
  ...
  $yatheme_data = array(
    ...
    array('{: $bar |  default   value   :}{:comment foo:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_ATTRIBUTE', '$bar'),
      array('Y_TEXT', 'default   value'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_OPEN_YBRACE', '{:'),
      array('error', 'file -:near line 1: Syntax Error: \'comment foo:}\''),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    
    // metadata and links
    array('{:meta content-type fluid:}{:comment foo:}', array(
      array('Y_OPEN_YBRACE', '{:'),
    ...
    array('{:script:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_SCRIPT', 'script'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array('{:script:}{:end-script:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_SCRIPT', 'script'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_SCRIPT', 'end-script'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array('{:script:}some javascript {:end-script:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_SCRIPT', 'script'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_TEXT', 'some javascript '),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_SCRIPT', 'end-script'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array("{:script:}some javascript <\x3fphp echo \"foo\";\x3f>{:end-script:}", array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_SCRIPT', 'script'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_TEXT', "some javascript <\x3fphp echo \"foo\";\x3f>"),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_SCRIPT', 'end-script'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    
    // FIXME: add style tests
    array('{:style:}{:end-style:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_STYLE', 'all'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_STYLE', 'end-style'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array('{:style screen :}{:end-style:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_STYLE', 'screen'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_STYLE', 'end-style'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array('{:style print:}some css {:end-style:}', array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_STYLE', 'print'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_TEXT', 'some css '),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_STYLE', 'end-style'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
    array("{:style:}some css <\x3fphp echo \"foo\";\x3f>{:end-style:}", array(
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_STYLE', 'all'),
      array('Y_CLOSE_YBRACE', ':}'),
      array('Y_TEXT', "some css <\x3fphp echo \"foo\";\x3f>"),
      array('Y_OPEN_YBRACE', '{:'),
      array('Y_END_STYLE', 'end-style'),
      array('Y_CLOSE_YBRACE', ':}'),
    )),
  ...
  $yatheme_parser_instance = new YAThemeParser(FALSE);
  $scanner = $yatheme_parser_instance->yatheme_scanner;
...
if ($test_attribute_parser) {
  $test_data = array(
    ...
    foreach ($test_data as $row) {
      ...
      $new_parser = $themer->phpvar_parser;
...
if ($test_yat_parser) {
  $test_data = array(
    ...
    array('{:guards on:}', '', 'return $yatheme_parser_instance->guards == "on";'),
    array('{:guards off:}', '', 'return $yatheme_parser_instance->guards == "off";'),
    array('{:yatemplate template-file.tpl:}', '', 'return $yatheme_parser_instance->template_file == "template-file.tpl";'),
    array('{:yatemplate-content:}', '{: yatemplate-content :}', 'return $yatheme_parser_instance->yatemplate_content == "";'),
    array('{:authority A,C,X:}', '', 'return $yatheme_parser_instance->required_authority == "A,C,X";'),
    array('{:errors ignore:}', '', 'return $yatheme_parser_instance->errors == "ignore";'),
    array('{:errors display:}', '', 'return $yatheme_parser_instance->errors == "display";'),
    array('{:errors email foo@example.com:}', '', 'return $yatheme_parser_instance->errors == "email" && $yatheme_parser_instance->errors_email == "foo@example.com";'),
    ...
    array('{:meta content-type eggplant:}', '', 'return $yatheme_parser_instance->meta[0] == array("content-type", "eggplant");'),
    array('{:render meta:}', '{:-meta-:}'),
    array('{:css /css/foo.css:}', '',
      'return $yatheme_parser_instance->css == array(array("link", "/css/foo.css", "all"));'),
    array('{:css /css/foo.css:}{:css /css/bar.css screen:}', '',
      'return $yatheme_parser_instance->css == array(array("link", "/css/foo.css", "all"), array("link", "/css/bar.css", "screen"));'),
    array('{:render css:}', '{:-css-:}'),
    array('{:javascript /js/js.js:}', '',
      'return $yatheme_parser_instance->javascript == array(array("link", "/js/js.js"));'),
    array('{:render javascript:}', '{:-javascript-:}'),
    array('{:style:}body {background:black;}{:end-style:}', '',
        'return $yatheme_parser_instance->css == array(array("style", "body {background:black;}", "all"));'),
  );
  ...
  foreach ($test_data as $row) {
    $yatheme_parser_instance = new YAThemeParser();
    $yatheme_parser_instance->guards = 'off';
    $parser = $yatheme_parser_instance->yatheme_parser;
    $scanner = $yatheme_parser_instance->yatheme_scanner;
...
if ($test_parser) {
  ...
  foreach ($scan_dir as $fname) {
    ...
    echo "\n";
    echo "==============Reading and Rendering '$basename'================\n";
    echo "==============Reading and Rendering '$basename'================\n";
    echo "==============Reading and Rendering '$basename'================\n";
